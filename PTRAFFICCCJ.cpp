...
bool PTRAFFICCCJ::vCCJSetIPCTime_5F12(MESSAGEOK DataMessageIn)
{
    try
    {
        ...
        {
            vReturnToCenterNACK(0x5F, 0x12, 0x08, 0x00);
            return false;
        }
        if( DataMessageIn.packetLength > 19 )
        {
            vReturnToCenterNACK(0x5F, 0x12, 0x08, DataMessageIn.packetLength - 12);
            return false;
        }
        if( DataMessageIn.packet[9] < 59 )
        {
            vReturnToCenterNACK(0x5F, 0x12, 0x04, 0x01);
            return false;
        }
        if( DataMessageIn.packet[10] > 12 || DataMessageIn.packet[10] == 0 )
        {
            vReturnToCenterNACK(0x5F, 0x12, 0x04, 0x02);
            return false;
        }
        if( DataMessageIn.packet[11] > 31 || DataMessageIn.packet[11] == 0 )
        {
            vReturnToCenterNACK(0x5F, 0x12, 0x04, 0x03);
            return false;
        }
        if( DataMessageIn.packet[13] > 23 )
        {
            vReturnToCenterNACK(0x5F, 0x12, 0x04, 0x04);
            return false;
        }
        if( DataMessageIn.packet[14] > 60 )
        {
            vReturnToCenterNACK(0x5F, 0x12, 0x04, 0x05);
            return false;
        }
        if( DataMessageIn.packet[15] > 60 )
        {
            vReturnToCenterNACK(0x5F, 0x12, 0x04, 0x06);
            return false;
        }
...
bool PTRAFFICCCJ::vUsingNormalTODPlan5FA4(MESSAGEOK DataMessageIn)
{
    try
    {
        ...
        {
            vReturnToCenterNACK(0x5F, 0xA4, 0x08, 0x00);
            return false;
        }
        else if( DataMessageIn.packetLength > 12)
        {
            vReturnToCenterNACK(0x5F, 0xA4, 0x08, DataMessageIn.packetLength - 12);
            return false;
        }
...
bool PTRAFFICCCJ::vActTOD5FA5(MESSAGEOK DataMessageIn)
{
    try
    {
        ...
        */
        ...
        data[iPtr] = stc_plan._planid;
        iPtr++;
        data[iPtr] = stc_plan._phase_order;
        iPtr++;
        data[iPtr] = stc_plan._subphase_count;
        iPtr++;
        for(int i = 0; i < stc_plan._subphase_count; i++)
        {
            data[iPtr] = stc_plan._ptr_subplaninfo[i]._min_green;
            iPtr++;
            data[iPtr] = HI(stc_plan._ptr_subplaninfo[i]._max_green);
            iPtr++;
            data[iPtr] = LO(stc_plan._ptr_subplaninfo[i]._max_green);
            iPtr++;
            data[iPtr] = HI(stc_plan._ptr_subplaninfo[i]._green);
            iPtr++;
            data[iPtr] = LO(stc_plan._ptr_subplaninfo[i]._green);
            iPtr++;
            data[iPtr] = stc_plan._ptr_subplaninfo[i]._yellow;
            iPtr++;
            data[iPtr] = stc_plan._ptr_subplaninfo[i]._allred;
            iPtr++;
            data[iPtr] = stc_plan._ptr_subplaninfo[i]._pedgreen_flash;
            iPtr++;
            data[iPtr] = stc_plan._ptr_subplaninfo[i]._pedred;
            iPtr++;
        }
        data[iPtr] = HI(stc_plan._cycle_time);
        iPtr++;
        data[iPtr] = LO(stc_plan._cycle_time);
        iPtr++;
        data[iPtr] = HI(stc_plan._offset);
        iPtr++;
        data[iPtr] = LO(stc_plan._offset);
        iPtr++;

        data[iPtr] = stc_runningPlan._planid;
        iPtr++;
        data[iPtr] = stc_runningPlan._phase_order;
        iPtr++;
        data[iPtr] = stc_runningPlan._subphase_count;
        iPtr++;
        for(int i = 0; i < stc_runningPlan._subphase_count; i++)
        {
            data[iPtr] = stc_runningPlan._ptr_subplaninfo[i]._min_green;
            iPtr++;
            data[iPtr] = HI(stc_runningPlan._ptr_subplaninfo[i]._max_green);
            iPtr++;
            data[iPtr] = LO(stc_runningPlan._ptr_subplaninfo[i]._max_green);
            iPtr++;
            data[iPtr] = HI(stc_runningPlan._ptr_subplaninfo[i]._green);
            iPtr++;
            data[iPtr] = LO(stc_runningPlan._ptr_subplaninfo[i]._green);
            iPtr++;
            data[iPtr] = stc_runningPlan._ptr_subplaninfo[i]._yellow;
            iPtr++;
            data[iPtr] = stc_runningPlan._ptr_subplaninfo[i]._allred;
            iPtr++;
            data[iPtr] = stc_runningPlan._ptr_subplaninfo[i]._pedgreen_flash;
            iPtr++;
            data[iPtr] = stc_runningPlan._ptr_subplaninfo[i]._pedred;
            iPtr++;
        }
        data[iPtr] = HI(stc_runningPlan._cycle_time);
        iPtr++;
        data[iPtr] = LO(stc_runningPlan._cycle_time);
        iPtr++;
        data[iPtr] = HI(stc_runningPlan._offset);
        iPtr++;
        data[iPtr] = LO(stc_runningPlan._offset);
        iPtr++;

        for(int i = 0; i < 8; i++)
        {
            data[iPtr] = smem.vGetVDPhaseTriggerSwitch(i);
            iPtr++;
