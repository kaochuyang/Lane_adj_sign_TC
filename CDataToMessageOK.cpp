        ...
        unsigned short int usiWay5 )
{
    try
    {
        ...
        _MESSAGEOUT.packet[iPtr] = 0xAA;
        iPtr++;
        _MESSAGEOUT.packet[iPtr] = 0xBB;
        iPtr++;

        if (smem.vGetAdjudicateReturnAddressBCDorHEX() == cBCD)                       //Select BCD mode
        {
            _MESSAGEOUT.packet[iPtr]=oHexToInt.vIntToFuckingBCD(1,_MESSAGEOUT.packetLcn );
            iPtr++;
            _MESSAGEOUT.packet[iPtr]=oHexToInt.vIntToFuckingBCD(0,_MESSAGEOUT.packetLcn );
            iPtr++;
        }
        else                                                                          //Normal(HEX) mode
        {
            _MESSAGEOUT.packet[iPtr]=HI(_MESSAGEOUT.packetLcn);
            iPtr++;
            _MESSAGEOUT.packet[iPtr]=LO(_MESSAGEOUT.packetLcn);
            iPtr++;
        }

        _MESSAGEOUT.packet[iPtr] = byteCMD;
        iPtr++;                                  //CMD

        _MESSAGEOUT.packet[iPtr] = usiWay1 & 0xFF;
        iPtr++;
        _MESSAGEOUT.packet[iPtr] = usiWay2 & 0xFF;
        iPtr++;
        _MESSAGEOUT.packet[iPtr] = usiWay3 & 0xFF;
        iPtr++;
        _MESSAGEOUT.packet[iPtr] = usiWay4 & 0xFF;
        iPtr++;
        _MESSAGEOUT.packet[iPtr] = usiWay5 & 0xFF;
        iPtr++;

        _MESSAGEOUT.packet[iPtr] = 0xAA;
        iPtr++;
        _MESSAGEOUT.packet[iPtr] = 0xCC;
        iPtr++;
